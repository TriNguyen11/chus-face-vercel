...
const Last = ({ img }) => {
  ...
  const downloadPcImg = async () => {
      html2canvas(document.getElementById("pre-image-downloads-pc").getElementsByTagName("img")[0], {}).then(
       (canvas) => {
  ...
  const downloadMbImg = async () => {
    html2canvas(document.getElementById("pre-image-downloads-mb").getElementsByTagName("img")[0], {}).then(
     (canvas) => {
      let cvs = document.createElement("canvas").appendChild(canvas);
    ...
    <>
      <div className="container-lg mx-auto h-screen ">
        ...
        <section className="grid grid-cols-12">
          <div id="pre-image-downloads-mb" className="md:hidden col-span-6 md:col-span-4 flex flex-col justify-end">
            <img
              ref={refMbImage}
